      ...
      <div class="col-xs-12">
        ...
        <div class="row">
          <div class="col-xs-12">
            ...
            <div class="table-header"> Results for "All FAQs" <span style="float:right; margin-right:8px;"> <a href="javascript:deleteAllfaq();" class="btn btn-danger btn-sm">Delete All</a> </span> <span style="float:right; margin-right:8px;">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="faq_label">Add FAQs</h4>
      </div>
      <div class="modal-body">
        <form role="form" id="faq_form">
		  ...
		  <div class="form-group">
			...
			$stmt = query(" SELECT * FROM `category` WHERE `category`.`type` = 'Faq' ORDER BY `category`.`title` ASC  ");
          ...
          <div class="form-group">
            <label for="email" class="control-label">Question :</label>
			<textarea style="width:100%;" rows="2" class="form-control" id="fq_qs" name="fq_qs"></textarea>
          </div>
          <div class="form-group">
            <label for="password" class="control-label">Answer:</label>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" id="faq_submit">Add faq</button>
      </div>
    </div>
  </div>
</div>
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data.status == 'success')
				{
					...
					if(faq_id)
					{
						$('#ajax-result').html('<div class="alert alert-success"><button data-dismiss="alert" class="close" type="button"><i class="icon-remove"></i></button><strong>Success! </strong>faq [ '+firstname+' '+lastname+' ] Edited Successfully ...!<br></div>');
					}
					else
					{
						$('#ajax-result').html('<div class="alert alert-success"><button data-dismiss="alert" class="close" type="button"><i class="icon-remove"></i></button><strong>Success! </strong>faq [ '+firstname+' '+lastname+' ] Added Successfully ...!<br></div>');
					}
...
function openEditfaq(faq_id)
{
	...
	$('#faq_label').html('Edit faq');
	$("#faq_submit").html('Edit faq');
...
function deletefaq(id)
{
	...
	ajaxreq('faq-ajax.php', "action=faq_delete&faq_id="+id, 'ajax-result');
...
function updatefaqGrid()
{
	...
	*/
	...
	ajaxreq('faq-ajax.php', "action=faq_view&pager=<?=$_REQUEST['pager']?>", 'faq_view');
