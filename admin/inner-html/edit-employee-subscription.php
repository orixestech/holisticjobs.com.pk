...
<form role="form" id="EditEmployeeSubscriptionForm">
  <input type="hidden" name="uid" value="<?=$uid?>" />
  <div class="modal-header no-padding">
    <div class="table-header"> Edit Employee Subscription Plan </div>
  </div>
  <div class="modal-body form-horizontal">
    <h4> Edit Subscription Details </h4>
  ...
  <div class="modal-footer no-margin-top">
    <button type="button" onclick="EditEmployeeSubscription()" class="btn btn-primary"> Submit </button>
  </div>
</form>
<script type="text/javascript">
	$('.ContentEditor').redactor({ autoresize: false });
	
	function EditEmployeeSubscription(){
		$("form#EditEmployeeSubscriptionForm").validationEngine('validate');
		var valid = $("#EditEmployeeSubscriptionForm .formError").length;
		if (valid != 0){ return false; }
	
		var form_data = $('form#EditEmployeeSubscriptionForm').serialize();
		...
		$.ajax({
			...
			success: function(data){
				$("form#EditEmployeeSubscriptionForm #Ajax-Result").html(data);
