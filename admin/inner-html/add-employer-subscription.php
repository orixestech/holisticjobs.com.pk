...
<form role="form" id="AddEmployerSubscriptionForm">
  <input type="hidden" name="PlanModule" value="Employer" />
  <div class="modal-header no-padding">
    <div class="table-header"> Add Employer Subscription </div>
  </div>
  ...
  <div class="modal-footer no-margin-top">
    <button type="button" onclick="AddEmployerSubscription()" class="btn btn-primary"> Submit </button>
  </div>
</form>
<script type="text/javascript">
	$('.ContentEditor').redactor({ autoresize: false });

	function AddEmployerSubscription(){
		var validate = $("form#AddEmployerSubscriptionForm").validationEngine('validate');
		var valid = $("#AddEmployerSubscriptionForm .formError").length;
		if (valid != 0){ return false; }
		
		var form_data = $('form#AddEmployerSubscriptionForm').serialize();
		...
		$.ajax({
			...
			success: function(data){
				$("form#AddEmployerSubscriptionForm #Ajax-Result").html(data);
