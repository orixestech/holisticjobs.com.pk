...

{
...

	{
		...
		Track('faq [ '.$firstname.' '.$lastname.' ] Edited Successfully ...!');
...

	{
		...
		Track('faq [ '.$firstname.' '.$lastname.' ] Added Successfully ...!');
...

{
	...
	if($num)
	{
		echo $message = Alert('success', 'FAQ Deleted Successfully ...!');
	}
}
...

{
	...
	{
		$return['status'] = 'success';
		$return['data'] = Alert('success', 'faq [ '.$firstname.' '.$lastname.' ] Status Updated Successfully ...!');
	}
	else
	{
...
<table class="table table-striped table-bordered table-hover ">
  <?php
	$paging = getPaging('faqs'," LEFT JOIN  `category` ON (`category`.`id` = `faqs`.`fq_cat`) WHERE (`category`.`type` = 'Faq' ) ORDER BY `category`.`title`, `faqs`.`fq_modifydate` DESC ",50,'faq-view.php','?',$_REQUEST['pager']);
    ...
    <tr id="row_<?=$rslt['fq_id']?>">
      ...
      <td><?php
				...
				$data = array(
						...
						'delete' => array('href'=>'javascript:void(0);', 'js'=>'onclick="if(confirm(\'Are you sure you want to delete this faq?\')){deletefaq('.$rslt['fq_id'].')}"')
