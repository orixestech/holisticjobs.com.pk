...
if($_GET['code']!=''){
	...
	if($Code['userid']>0){
		$SQL = " SELECT * FROM `employee` WHERE  UID = '".$Code['userid']."' ";
		...
		if($row){
			...
			switch ($Code['type']) {
				case 'UpdateAccountEmailSQL':
					...
					$updateSQL = "update employee set EmployeeEmail = '".$Code['newemail']."' where `UID` = '".$Code['userid']."' ";
					query($updateSQL);
					$message = '
					...
					$data['addAddress'] = array($Code['newemail']=>$rslt['EmployeeName']);
