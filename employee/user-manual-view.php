<?php include('header.php');?>
    ...
    <div class="main-content">
      <div class="breadcrumbs" id="breadcrumbs">
        ...
        <!-- .breadcrumb --> 
		<span class="pull-right"><?=$SubscriptionExpireStatus?></span>
      </div>
                ...
                <div class="widget-body">
                  <div class="widget-main padding-8">
                    <div class="tree tree-selectable">
                      ...
                      <?php
					  $cnt = 0;
                      $stmt = query(" SELECT * FROM `category` WHERE `catid` = 0 and `type` = 'User-Manual-Employee' order by title; ");
					  while( $rslt = fetch($stmt) ){ if($cnt==0){ $LoadFirstCat = $rslt['id']; } $cnt++;
						  $subCatSql = "SELECT * FROM `category` WHERE `catid` = '".$rslt['id']."' and `type` = 'User-Manual-Employee' order by title;";
