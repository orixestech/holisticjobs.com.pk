...
if($_GET["status"] && $_GET["code"]){
	...
	mysql_query(" UPDATE `jobs` SET `JobStatus` = 'Publish' WHERE `jobs`.`UID` = '".$_GET["code"]."' ");
	$num = mysql_affected_rows();
	if($num){
		Track('Job [ '.$pageTitle.' ] Has been Published...!');
		$message = Alert('success', 'Job [ '.$pageTitle.' ] Has been Published...!');
	}
}
...
$QUERYSTRING = '';
$whereSQL = "where JobEmployerID = '".$_SESSION['EmployerUID']."' and JobLastDateApply >= '".date("Y-m-d")."' and JobStatus = 'Un-Publish' ";
...
<div class="main-content">
  <div class="breadcrumbs" id="breadcrumbs">
    <ul class="breadcrumb">
      ...
      <li class="active">Unpublished Jobs</li>
    </ul>
  ...
  <div class="page-content">
    ...
    <div class="row">
      <div class="col-xs-12">
        <div class="col-xs-12">
          <h3 class="header smaller lighter blue">Unpublished Jobs</h3>
          <div id="ajax-result">
            <?=$message?>
          </div>
          <div class="col-xs-12">
              ...
              <div class="widget-body">
                <div class="widget-main">
                  <form class="form-inline" id="admin-meeting-filter-form" method="get">
                    ...
                    <div class="col-xs-1 col-sm-1"> <a href="unpublished-jobs.php" style="margin-top:24px;" class="btn btn-success btn-sm"> Clear </a> </div>
                  </form>
          ...
          <div class="col-xs-12">
            ...
            <div class="table-responsive">
              ...
              $paging = getPaging('jobs',$whereSQL . " ORDER BY `SystemDate` DESC ",$limit,'unpublished-jobs.php','?'.$QUERYSTRING,$_REQUEST['pager']);
              //echo $paging[0];
            ...
            if($row>0){
              while( $rslt = mysql_fetch_array($rs_pages) ){
                  ...
                  <tr>
                    ...
                    <td><?=$rslt['JobNumbOfVacancy']?>  </td>
                    <td><?=JobExtra($rslt['UID'], 'JobCity', 'string')?></td>
                    <td><?=$rslt['JobLastDateApply']?></td>
                    <td> 
					<?php if ($totalApplicatoin > 0 ){ ?>
                    		<a href="job-applications.php?uid=<?=$rslt["UID"]?>"><strong><?=$totalApplicatoin?></strong> Applications</a>
					<?php } else { ?>
                    		N/A
					<?php }?> </td>
                    <td><?php
                  $data = array(
				  			  array('title'=>'<i class="icon-envelope bigger-130"></i>', 'href'=>'unpublished-jobs.php?status=Publish&code='.$rslt['UID'], 'js'=>' role="button" class="green" title="Click there to Publish this Job."'),
							  ...
							  array('title'=>'<i class="icon-trash bigger-130"></i>', 'href'=>'javascript:void(0)', 'js'=>' onclick="ConfirmDelete(\'unpublished-jobs.php?delete=true&id='.$rslt['UID'].'\')" class="red ConfirmDelete"  title="Delete" ')
